...
async def _get(session: aiohttp.ClientSession, sem: asyncio.Semaphore, path: str, params: dict) -> dict:
    """Send a single GET request, respecting the semaphore and rate limit."""
    url = BASE_URL + path
...
async def _fetch_slugs(session: aiohttp.ClientSession, sem: asyncio.Semaphore, page: int) -> list[str]:
    """Return job slugs from a single listing page."""
    logger.info('Fetching slugs — page %d', page)
    data = await _get(session, sem, '/pages/jobs', params={'page': page})
    return [job['slug'] for job in data['result']['list']]


async def _fetch_vacancy(session: aiohttp.ClientSession, sem: asyncio.Semaphore, slug: str) -> dict:
    """Return raw vacancy dict for a given slug."""
    logger.info('Fetching vacancy — slug=%s', slug)
    data = await _get(session, sem, '/pages/job', params={'slug': slug})
...
async def run_extract(pages: int) -> list[dict]:
    ...
    async with aiohttp.ClientSession() as session:
        slug_tasks = [_fetch_slugs(session, sem, page) for page in range(1, pages + 1)]
        ...
        vacancy_tasks = [_fetch_vacancy(session, sem, slug) for slug in slugs]
